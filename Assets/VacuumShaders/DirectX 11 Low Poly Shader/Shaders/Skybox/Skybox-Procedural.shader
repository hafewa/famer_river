Shader "Hidden/VacuumShaders/DirectX 11 Low Poly/Skybox/Procedural" 
{
Properties 
{
	[VacuumShadersSkyboxType] _Skybox_Type("", Float) = 1

	[KeywordEnum(None, Simple, High Quality)] _SunDisk ("Sun", Int) = 0
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			half3 vertex 			= -v.vertex;
		#elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
			half3 rayDir 			= half3(-eyeRay);
		#else
			half skyGroundFactor	= -eyeRay.y / SKY_GROUND_THRESHOLD;
		#endif
			...
			half3 skyColor	= _Exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));

		#if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
			half3 sunColor	= _Exposure * (cOut * _LightColor0.xyz);
		#endif
...
/////////////////////////////////////////////////////////////////////////////////////////////
		...
		#if SKYBOX_SUNDISK == SKYBOX_SUNDISK_HQ
			half3 ray = normalize(mul((float3x3)unity_ObjectToWorld, vertex));
			half y = ray.y / SKY_GROUND_THRESHOLD;
		#elif SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
			half3 ray = rayDir.xyz;
			half y = ray.y / SKY_GROUND_THRESHOLD;
		#else
		...
		half4 frag (v2f_surf IN) : SV_Target
		{
			...
			//PaperCraft
//			MakePaperCraft(IN.mass, IN.worldPos, finalColor);

			#if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
				if(IN.yRay.w < 0.0)
				{
					#if SKYBOX_SUNDISK == SKYBOX_SUNDISK_SIMPLE
						half mie = calcSunSpot(_WorldSpaceLightPos0.xyz, -IN.yRay.xyz);
					#else // SKYBOX_SUNDISK_HQ
						half eyeCos = dot(_WorldSpaceLightPos0.xyz, normalize(IN.yRay.xyz));
						half eyeCos2 = eyeCos * eyeCos;
						half mie = getMiePhase(eyeCos, eyeCos2);
					#endif

					finalColor.rgb += mie * IN.sunColor;
				}
			#endif
