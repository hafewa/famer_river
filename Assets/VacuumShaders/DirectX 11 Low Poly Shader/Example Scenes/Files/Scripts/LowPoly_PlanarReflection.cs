...
namespace VacuumShaders
{
    ...
    {
        [ExecuteInEditMode]
        public class LowPoly_PlanarReflection : MonoBehaviour
        {
            string reflectionSampler = "_ReflectionTex";
            ...
            static public LowPoly_PlanarReflection get;
            ...
            {
                String reflName = "Low Poly Reflection Camera (" + gameObject.name + ")";
            ...
            {
                RenderHelpCameras(Camera.current);

                if (m_ReflectionCamera && targetMaterial)
                {
                    targetMaterial.SetTexture(reflectionSampler, m_ReflectionCamera.targetTexture);
                }
            }
            ...
            void RenderReflectionFor(Camera cam, Camera reflectCamera)
            {
                ...
                if (targetMaterial && !targetMaterial.HasProperty(reflectionSampler))
                {
                    return;
                }
                ...
                float d = -Vector3.Dot(normal, pos) - (clipPlaneOffset < 0 ? 0 : clipPlaneOffset);
            ...
            {
                Vector3 offsetPos = pos + normal * (clipPlaneOffset < 0 ? 0 : clipPlaneOffset);
